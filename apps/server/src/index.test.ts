...
import { createTestDb } from "@uchi/db";
  ...
  describe("Tasks", () => {
    ...
    describe("POST /api/tasks", () => {
      it("タスクを作成して 201 を返す", async () => {
        ...
        const data = await res.json() as { id: number; title: string; date: string };
  ...
  describe("Reviews", () => {
    ...
    describe("DELETE /api/reviews/:week", () => {
      it("レビューを削除できる", async () => {
        ...
        const res = await app.request("/api/reviews/2026-W22", { method: "DELETE" });
        expect(res.status).toBe(200);
      ...
      it("存在しない週は 404 を返す", async () => {
        const res = await app.request("/api/reviews/2000-W01", { method: "DELETE" });
        expect(res.status).toBe(404);
      });
    });
  });
  ...
  describe("Goals", () => {
    describe("GET /api/goals", () => {
      ...
      it("未達成の目標のみ返す（デフォルト）", async () => {
        const g1 = await (await app.request("/api/goals", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    describe("DELETE /api/goals/:id", () => {
      it("目標をソフトデリートして deletedAt をセットする", async () => {
        ...
        const res = await app.request(`/api/goals/${goal.id}`, { method: "DELETE" });
        expect(res.status).toBe(200);
        const data = await res.json() as { deletedAt: string | null };
        expect(data.deletedAt).not.toBeNull();
      });
