...
function DropZone({
  dateKey, onMove, className, children, style, onClick,
}: {
  dateKey: string;
  ...
  return (
    ...
    // biome-ignore lint/a11y/useKeyWithClickEvents: calendar column click is optional; keyboard navigation handled at higher level
    <div
...
export function WeekView({ weekStart, ctx, layout, showWeekend }: Props) {
  ...
  const days = Array.from({ length: dayCount }, (_, i) => DateU.addDays(weekStart, i));
  ...
  if (layout === 'columns') {
    return (
      <div className="wk wk-columns" style={{ gridTemplateColumns: `repeat(${dayCount},1fr)` }}>
        {days.map((date) => {
          const k = DateU.key(date);
      ...
      {days.map((date) => {
        ...
        return (
          <DropZone key={k} dateKey={k} onMove={ctx.moveTo}
            ...
            <div className={`row-head${dow === 6 ? ' sun' : ''}${dow === 5 ? ' sat' : ''}`}>
              ...
              {st.total > 0 && <span className="row-frac">{st.done}/{st.total}</span>}
            </div>
