...
export async function fetchReviews(): Promise<Review[]> {
	...
	return parseOrThrow<InferResponseType<ReviewsGet, 200>>(res, "Failed to fetch reviews");
}
...
export async function fetchReview(week: string): Promise<Review | null> {
	...
	return parseOrThrow<InferResponseType<ReviewGet, 200>>(res, "Failed to fetch review");
}
...
export async function upsertReview(week: string, content: string): Promise<Review> {
	...
	return parseOrThrow<InferResponseType<ReviewPut, 200>>(res, "Failed to save review");
}
