...
import { makeWeeklyCycle, setMockWeeklyCycles } from "../test/handlers";
...
describe("ReviewPage", () => {
  it("今週の目標と振り返りが表示される", async () => {
    ...
    expect(
      await screen.findByDisplayValue("今週の目標内容"),
    ).toBeInTheDocument();
    expect(
  ...
  it("取得エラー時にエラーメッセージを表示する", async () => {
    server.use(
      http.get("/api/weekly-cycles/:week", () =>
  ...
  it("過去の週次サイクル一覧が表示される", async () => {
    setMockWeeklyCycles([
      makeWeeklyCycle({
        id: 1,
        week: "2026-W22",
        goalContent: "先週の目標",
        reviewContent: "先週の振り返り",
