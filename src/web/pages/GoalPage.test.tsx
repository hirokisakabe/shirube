...
describe("GoalPage", () => {
	...
	it("「達成済みを表示」フィルタ切替で達成済みの目標を表示する", async () => {
		setMockGoals([makeGoal({ id: 1, title: "達成済み目標", doneAt: "2026-06-01T12:00:00.000Z" })]);
	...
	it("削除ボタンで目標が消え、失敗時に復元される", async () => {
		...
		server.use(http.delete("/api/goals/:id", () => HttpResponse.json({ error: "network error" }, { status: 500 })));
	...
	it("取得エラー時にエラーメッセージを表示する", async () => {
		server.use(http.get("/api/goals", () => HttpResponse.json({ error: "network error" }, { status: 500 })));
