...
export function CalendarPage() {
  ...
  const monthAnchor = view === 'month'
    ? new Date(anchor.getFullYear(), anchor.getMonth(), 1)
    : anchor;

  const goPrev = () => setAnchor(DateU.addDays(anchor, view === 'week' ? -7 : -30));
  const goNext = () => setAnchor(DateU.addDays(anchor, view === 'week' ? 7 : 30));
  const goToday = () => setAnchor(DateU.today());

  const pickDay = (date: Date) => { setAnchor(date); setView('week'); };

  if (ctx.loading) return <div style={{ padding: 32, color: 'var(--ink-soft)' }}>読み込み中…</div>;
  if (ctx.error) return <div style={{ padding: 32, color: 'var(--accent)' }}>エラー: {ctx.error}</div>;
